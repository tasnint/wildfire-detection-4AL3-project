...
# ===============================
# 2. PREPROCESSING (From experiments or fixed size)
# ===============================
# Using fixed size for this example - you can integrate preprocessing experiments
IMG_SIZE = (224, 224)
BATCH_SIZE = 64
...
def basic_block(x, filters, stride=1, name=None):
    ...
    x = layers.Conv2D(filters, (3, 3), strides=1, padding='same', name=f'{name}_conv2')(x)
    x = layers.BatchNormalization(name=f'{name}_bn2')(x)
    
    if stride != 1 or shortcut.shape[-1] != filters:
        shortcut = layers.Conv2D(filters, (1, 1), strides=stride, padding='same', name=f'{name}_shortcut_conv')(shortcut)
...
def resnet_stage(x, filters, num_blocks, stride, stage_name):
    """ResNet stage with multiple basic blocks."""
...
# ===============================
# 4. BUILD RESNET18 MODEL WITH ARCHITECTURAL DETAILS
# ===============================
def build_resnet18(input_shape, num_classes=1):
    ...
    print(f"\nArchitecture:")
    print(f"  Base: ResNet18 (trained from scratch)")
    print(f"  Stem: Conv2D(7x7, 64) -> BN -> ReLU -> MaxPool(3x3)")
    print(f"  Stage 1: 2 blocks, 64 filters")
    print(f"  Stage 2: 2 blocks, 128 filters, stride=2")
    print(f"  Stage 3: 2 blocks, 256 filters, stride=2")
    print(f"  Stage 4: 2 blocks, 512 filters, stride=2")
    print(f"  Head: GlobalAvgPool -> Dropout(0.5) -> Dense(1, Sigmoid)")
    print(f"  Residual connections: Skip connections in each block")
    print("="*80 + "\n")
    
    return model

input_shape = (IMG_SIZE[0], IMG_SIZE[1], 3)
model = build_resnet18(input_shape, 1)
...
plt.tight_layout()
plt.savefig("models/resnet18_confusion_matrix.png", dpi=150, bbox_inches='tight')
plt.close()

report = classification_report(y_true, y_pred, target_names=["No Fire", "Fire"], output_dict=True)
df_report = pd.DataFrame(report).transpose()
print(f"\nClassification Report:\n{df_report.round(3)}")
df_report.to_csv("results/resnet18_classification_report.csv")
...
plt.tight_layout()
plt.savefig("models/resnet18_roc_pr_threshold_analysis.png", dpi=150, bbox_inches='tight')
plt.close()
...
3. TRANSFER LEARNING (Medium-term):
   • Use pretrained ResNet18 on ImageNet
   • Fine-tune for wildfire detection
...
5. ENSEMBLE METHODS (Medium-term):
   • Combine ResNet18 + EfficientNet + MobileNet
   • Voting/averaging reduces model-specific errors

6. ARCHITECTURE IMPROVEMENTS (Long-term):
   • Try ResNet50, ResNet101 for deeper features
   • Add attention mechanisms (CBAM, SE blocks)
   • Multi-scale feature fusion for small fires
...
# ===============================
model.save("models/resnet18_wildfire.h5")
print("\n✅ Model saved to models/resnet18_wildfire.h5")
...
plt.tight_layout()
plt.savefig("models/resnet18_training_curves.png", dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig("models/resnet18_sample_predictions.png", dpi=150, bbox_inches='tight')
plt.close()
